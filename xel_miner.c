...
static int execute_vm(int thr_id, struct work *work, struct instance *inst, long *hashes_done, char* hash, bool new_work) {
    ...
    while (1) {
        ...
        // Reset VM Memory on positions 12-34 (BOUNTY_STORAGE_INTS ints storage)
        memset((void*)vm_m + 12*sizeof(int), 0, BOUNTY_STORAGE_INTS * sizeof(int));

        // copy work combined_storage into M array
        memcpy((void*)vm_m + (12 + BOUNTY_STORAGE_INTS)*sizeof(int), work->combined_storage, work->combined_storage_size * sizeof(int));
        ...
        if (rc == 1){
             ...
             printf("Dumping mid30 int elements of WORK m\n");
            for(i=30;i<60;++i)
                printf("%d: %d\n",i, vm_m[i]);

            // Do not forget to save the storage
            for(i=0;i<BOUNTY_STORAGE_INTS;++i)
                work->storage_ints[i] = vm_m[12+i];
            return rc;
        }
...
static int work_decode(const json_t *val, struct work *work) {
    ...
    for (i = 0; i<num_pkg; i++) {
        ...
        if (work_pkg_id < 0) {
            ...
            work_package.iterations = (uint32_t)json_integer_value(json_object_get(pkg, "repetitions"));
            ...
            work_package.combined_storage_size = work_package.iterations * work_package.bounty_limit;
            work_package.combined_storage = (int32_t*)malloc(sizeof(int32_t) * work_package.combined_storage_size);
            memset((char*)work_package.combined_storage, 0, sizeof(int32_t) * work_package.combined_storage_size);
        ...
        strcpy(parsetok, str+1);
        parsetok[strlen(str)-1] = 0;
        char *pt;
        pt = strtok (parsetok,",");
        j = 0;
        while (pt != NULL) {
            int a = atoi(pt);
            g_work_package[work_pkg_id].combined_storage[j] = a;
            ++j;
            pt = strtok (NULL, ",");
        }
    ...
    if(work->combined_storage != 0)
        free(work->combined_storage);
    work->combined_storage = (int32_t*)malloc(sizeof(int32_t) * g_work_package[best_pkg].combined_storage_size);
    memcpy(work->combined_storage, g_work_package[best_pkg].combined_storage, sizeof(uint32_t)*g_work_package[best_pkg].combined_storage_size);
