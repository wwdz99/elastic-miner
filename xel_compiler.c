...
#ifdef WIN32
	fprintf(f, "__declspec(dllexport) void initialize(int32_t *vm_m, double *vm_f, uint32_t *vm_tmp, uint32_t *vm_state) {\n");
#else
	fprintf(f, "void initialize(int32_t *vm_m, double *vm_f, mpz_t *vm_b, uint32_t *vm_state) {\n");
#endif
	fprintf(f, "\tm = vm_m;\n");
	fprintf(f, "\tf = vm_f;\n");
	fprintf(f, "\ttmp = vm_tmp;\n");
...
void create_instance(struct instance* inst, char *lib_name) {
	...
	inst->initialize = (int(__cdecl *)(int32_t *, double *, uint32_t *, uint32_t *))GetProcAddress((HMODULE)inst->hndl, "initialize");
